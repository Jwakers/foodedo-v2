...
**Shop — MVP foundation complete:** a guest can preview the ingredients derived from the temporary plan on that device. Every authenticated meal plan owns one durable list, with the active plan's list shown by default and previous-plan lists available through a compact date selector for delayed shopping. Plan writes reconcile their linked list in the same transaction: exact-name ingredients that remain preserve user state, obsolete derived items disappear, new items arrive unchecked, and manual extras are untouched. Users can check items, add extras, and soft-remove/restore items with Convex optimistic cache updates. Exact normalised ingredient names are grouped while each source quantity remains visible. A list follows its meal plan's lifetime rather than expiring independently.
...
The current catalogue uses real `/recipes/[slug]` pages. A catalogue slug is a unique, human-readable URL value distinct from the stable internal catalogue ID used by plans and saved snapshots. Every bundled slug is generated at build time via `generateStaticParams`, so the same URLs work on Vercel and in Capacitor's static iOS bundle without a query-string router. That per-slug HTML growth is acceptable at the current ~30-meal catalogue; it is a separate Capacitor build-time and bundle-size risk from shipping meal JSON and images, and it must not be extended to personal recipes. When the catalogue moves to Convex public reads—or before hundreds of prebuilt pages become costly—prefer a client-hydrated native detail shell (web may keep SEO SSG). See [recipes-and-ingredients.md](../knowledge/architecture/recipes-and-ingredients.md). Routes for future user-created recipes will be designed with duplicate handling and that native static-export constraint when Capture adds them; they are not simulated ahead of the data source.
