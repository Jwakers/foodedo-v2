...
- [ ] Verify shopping checks/removal are optimistic and roll back on mutation failure; removed items can be restored. Prove plan changes reconcile the same linked list while preserving retained checks and manual items. Prove account deletion removes child shopping items before their parent without crossing account boundaries. When an explicit future plan-deletion flow exists, prove it uses the same child-before-parent order; until then, treat that check as deferred.
- [ ] On Shop, select a previous plan from the date selector, confirm the selection is reflected in the URL, edit that historical list (check/uncheck or remove/restore), then return to the current list and confirm it defaults to the active plan’s list. If there is no active plan but retained lists exist, confirm a requested or recent historical list remains reachable.
